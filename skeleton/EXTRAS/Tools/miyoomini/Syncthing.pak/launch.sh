...
check_injector() {
    if grep -q "#SYNCTHING INJECTOR" "/mnt/SDCARD/.system/miyoomini/paks/MinUI.pak/launch.sh"; then
        return 0
    else
        return 1
    fi
...
injectruntime() {
    build_infoPanel "Injecting config into launch.sh..."
    sed -i '/# Auto launch/i \\	sh /mnt/SDCARD/Tools/miyoomini/Syncthing/script/checkrun.sh #SYNCTHING INJECTOR #SYNCTHING INJECTOR' /mnt/SDCARD/.system/miyoomini/paks/MinUI.pak/launch.sh
